...
const rankSearcResult = (testString: string, queryString: string): number => {
    if (queryString == null || queryString.length === 0) {
        return 0;
    } if (matchesStartOfWord(testString, queryString)) {
        return 3;
    } if (matchesStartOfString(testString, queryString)) {
        return 2;
    } if (matchesAnywhere(testString, queryString)) {
        return 1;
    } 
        return 0;
